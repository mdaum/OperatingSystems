...
struct trie_node * new_leaf (const char *string, size_t strlen, int32_t ip4_address) { //only time node is ever created....
  ...
  assert(pthread_mutex_lock(&node_count_mutex)==0);//acquire count lock
  node_count++;
  assert(pthread_mutex_unlock(&node_count_mutex)==0);//release count lock
  if (!new_node) {
...
void handle_delete_thread(){ //called from main delete_thread
	assert(pthread_mutex_lock(&trie_mutex)==0); //must first acquire lock
...
_search (struct trie_node *node, const char *string, size_t strlen) { //root is locked prior to entering this method from search...
  int keylen, cmp;
  if(node==root)assert(pthread_mutex_unlock(&rootex)==0);//unlock root pointer if it is node...i.e will unlock upon first entry to method....
...
int _insert (const char *string, size_t strlen, int32_t ip4_address, 
  ...
  int cmp, keylen;
 if(node==root)assert(pthread_mutex_unlock(&rootex)==0);//unlock root pointer if it is node...this will happen on first entry to this method..
  // First things first, check if we are NULL 
  assert (node != NULL);
  assert (node->strlen < 64);
  ...
  if (cmp == 0) {
	...
	left->next = new_node;
		...
		assert(pthread_mutex_lock(&rootex)==0);//acquire root pointer lock first
...
int insert (const char *string, size_t strlen, int32_t ip4_address) { //INTERFACE	
  ...
  assert(pthread_mutex_lock(&root->node_lock)==0);//first lock root outside of recursive function
...
//puts("in drop node");
  assert(node_count > max_count);
    ...
    assert(pthread_mutex_lock(&rootex)==0);//lock root pointer	
	assert(pthread_mutex_lock(&root->node_lock)==0);//lock root node
	struct trie_node *temp = root;
	//struct trie_node *currRoot=root;
